...
async function saveOrderIntent(supabase, cfg, orderId, acceptedAt = new Date().toISOString()) {
  const row = {
  ...
  try {
    ...
    if (cfg.env === 'live') {
      ...
      const creditNoticeHtml = isDiscovery
        ? '<p><strong>Vos 29 € seront déduits</strong> si vous poursuivez ensuite avec la Formation complète.</p>'
        : ''
      const creditNoticeText = isDiscovery
        ? '\n\nVos 29 € seront déduits si vous poursuivez ensuite avec la Formation complète.'
        : ''
...
async function recoverOrCapture(cfg, orderId) {
  if (!isSupabaseConfigured()) throw new Error('supabase_not_configured')
  const supabase = getSupabaseAdmin()
...
async function handle(req, res, action, forcedEnv = null) {
  ...
  if (action === 'create') {
    ...
    try {
      ...
      if (!isSupabaseConfigured()) throw new Error('supabase_not_configured')
      await saveOrderIntent(getSupabaseAdmin(), cfg, data.id)
