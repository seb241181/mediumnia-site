...
export function buildRdvDepositConfirmation({
  ...
  const paidInFull = depositCents >= servicePriceCents
  const balanceCents = Math.max(0, servicePriceCents - depositCents)
  const subject = 'MediumIA — Votre rendez-vous est confirmé'
  const paymentIntro = paidInFull
    ? 'Votre prestation a bien été réglée intégralement. Votre créneau est maintenant confirmé.'
    : 'Vos arrhes de réservation ont bien été réglées. Votre créneau est maintenant confirmé.'
  const paymentLabel = paidInFull ? 'Réglé intégralement' : 'Arrhes réglées'
  const cancellationNote = paidInFull
    ? 'À moins de 48 heures, l’annulation automatique est bloquée. Le traitement d’un paiement intégral dépend des conditions de réservation acceptées et des droits légaux applicables.'
  ...
  const balanceReminderHtml = !paidInFull && balanceReminderEnabled && balanceCents > 0
    ? `<div style="margin:18px 0;padding:15px 17px;border:1px solid #DEC77F;background:#FFF9E9;border-radius:10px;font-size:13px;line-height:1.65;color:#4A3F6B;"><strong>Solde à régler avant la séance :</strong> ${money(balanceCents)}.<br>MediumIA vous enverra un rappel avec un lien de paiement sécurisé environ 72 heures avant le rendez-vous. Le solde devra être reçu au plus tard 48 heures avant la séance ; à défaut, le rendez-vous sera automatiquement annulé et le créneau libéré.</div>`
    : ''
  const balanceReminderText = !paidInFull && balanceReminderEnabled && balanceCents > 0
    ? `MediumIA vous enverra un rappel avec un lien de paiement sécurisé environ 72 heures avant le rendez-vous. Le solde de ${money(balanceCents)} devra être reçu au plus tard 48 heures avant la séance ; à défaut, le rendez-vous sera automatiquement annulé et le créneau libéré.`
    : null
...
<div style="max-width:580px;margin:0 auto;padding:40px 24px;">
  ...
  <p style="font-size:15px;line-height:1.7;">${escapeHtml(paymentIntro)}</p>
  <div style="background:#F0EDE8;border-radius:12px;padding:20px 24px;margin:22px 0;">
    ...
    <p style="margin:4px 0;font-size:14px;color:#4A3F6B;"><strong>${paymentLabel} :</strong> ${money(depositCents)}</p>
  ...
  <p style="font-size:12px;line-height:1.6;color:#756B89;">${escapeHtml(cancellationNote)}</p>
  ...
  const text = [
    ...
    `${paymentLabel} : ${money(depositCents)}`,
    `Solde restant : ${money(balanceCents)}`, '',
    ...(balanceReminderText ? [balanceReminderText, ''] : []),
