...
export default async function handler(req, res) {
  ...
  if (!isSupabaseConfigured()) {
    return res.status(200).json({ mode: 'demo', slots: generateDemoSlots(new Date(date + 'T12:00:00Z')), notice: 'Créneaux de démonstration — Supabase non configuré.' })
  }
  ...
  const { data: practitioner } = await supabase
    ...
    .select('id, timezone, buffer_before_min, buffer_after_min')
    .eq('slug', slug)
    .single()

  if (!practitioner) {
    return res.status(200).json({ mode: 'demo', slots: generateDemoSlots(new Date(date + 'T12:00:00Z')), notice: 'Praticien introuvable — créneaux de démonstration.' })
  }
  ...
  if (!conn) {
    return res.status(200).json({ mode: 'demo', slots: generateDemoSlots(new Date(date + 'T12:00:00Z')), notice: 'Google Agenda non connecté — créneaux de démonstration.' })
  }

  // ── Service ───────────────────────────────────────────────────────────────

  if (!serviceSlugParam) {
    return res.status(200).json({ mode: 'configuration_required', slots: [], notice: 'service_slug requis en mode LIVE.' })
  }
  ...
  if (!svc) {
    return res.status(200).json({ mode: 'configuration_required', slots: [], notice: 'Service introuvable ou inactif.' })
  }
  ...
  if (exception?.exception_type === 'modified' && exception.slots?.length) {
    ...
    if (rulesErr || !dbRules?.length) {
      return res.status(200).json({ mode: 'configuration_required', slots: [], notice: 'Les horaires de disponibilité ne sont pas encore configurés pour ce jour.' })
    }
    rules = dbRules
  }
