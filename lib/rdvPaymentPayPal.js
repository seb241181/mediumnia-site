...
const SANDBOX = {
  ...
  referenceId: 'MEDIUMIA_RDV_PAYMENT_SANDBOX',
}

const LIVE = {
  env: 'live',
  base: 'https://api-m.paypal.com',
  referenceId: 'MEDIUMIA_RDV_PAYMENT',
}

export function formatRdvPaymentAmount(cents) {
  if (!Number.isInteger(cents) || cents <= 0) throw new Error('rdv_payment_amount_invalid')
  return (cents / 100).toFixed(2)
}

export function isPayableRdvService(service, selectedModality) {
...
export function runtimeRdvPaymentPayPalConfig() {
  if (process.env.VERCEL_ENV === 'production') {
    if (process.env.PAYPAL_RDV_DEPOSIT_ENABLED !== 'true') throw new Error('rdv_payment_paypal_disabled')
    if (process.env.PAYPAL_RDV_DEPOSIT_ENV !== 'live') throw new Error('rdv_payment_paypal_live_not_configured')
    return LIVE
  }
  return SANDBOX
}
...
export function rdvPaymentCustomId(holdId) {
  return `MEDIUMIA:RDV-PAYMENT:${holdId}`
}

export function rdvPaymentCreateRequestId(holdId) {
  return `rdv-payment-create-${holdId}`.slice(0, 108)
}
...
export async function rdvPaymentPayPalConfig() {
  const cfg = runtimeRdvPaymentPayPalConfig()
  const { clientId } = credentials()
  return { clientId, env: cfg.env, currency: 'EUR' }
}

export async function createRdvPayPalOrder({ holdId, amountCents, currency, serviceTitle, paymentOption }) {
  const cfg = runtimeRdvPaymentPayPalConfig()
  if (currency !== 'EUR') throw new Error('rdv_payment_currency_invalid')
  if (!['deposit', 'full'].includes(paymentOption)) throw new Error('invalid_payment_option')

  const accessToken = await getAccessToken(cfg)
  const requestId = rdvPaymentCreateRequestId(holdId)
  const paymentLabel = paymentOption === 'full' ? 'Paiement intégral' : 'Arrhes de réservation'
  const body = JSON.stringify({
    intent: 'CAPTURE',
    purchase_units: [{
      reference_id: cfg.referenceId,
      custom_id: rdvPaymentCustomId(holdId),
      description: `MediumIA — ${paymentLabel}${serviceTitle ? ` — ${serviceTitle}` : ''}`.slice(0, 127),
      amount: { currency_code: currency, value: formatRdvPaymentAmount(amountCents) },
    }],
    payment_source: {
      paypal: {
        experience_context: {
...
export async function captureRdvPayPalOrder(orderId) {
  ...
  const response = await fetch(`${cfg.base}/v2/checkout/orders/${encodeURIComponent(orderId)}/capture`, {
    ...
    headers: {
      Authorization: `Bearer ${accessToken}`,
      'Content-Type': 'application/json',
      'PayPal-Request-Id': `rdv-payment-capture-${orderId}`.slice(0, 108),
    },
  })
...
export function verifyRdvPayPalPayment({ cfg, data, holdId, amountCents, currency }) {
  ...
  const expectedAmount = formatRdvPaymentAmount(amountCents)

  if (!unit || !capture?.id || !data.id) throw new Error('paypal_payment_invalid')
  if (unit.custom_id != null && unit.custom_id !== rdvPaymentCustomId(holdId)) throw new Error('paypal_custom_id_mismatch')
