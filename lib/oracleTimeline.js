...
export async function handleOracleTimeline(req, res) {
  ...
  if (isPack) {
    const { data: packMeta, error: packMetaError } = await supabase.from('chronosphere_credit_packs')
      .select('id, user_id, product_type, created_at, captured_at')
      .eq('pack_token_hash', tokenHash)
      .maybeSingle()
    if (packMetaError) return res.status(503).json({ error: 'pack_status_unavailable' })
    if (packMeta && isPackExpired(packMeta)) {
      return res.status(403).json({
        error: 'draw_token_expired',
        message: 'Ce pack a dépassé sa durée de validité de 6 mois : les tirages non utilisés ne sont plus disponibles.',
      })
    }
    packExpiry = packMeta ? packExpiresAt(packMeta) : null
  ...
  if (tokenResult.cached && tokenResult.result_json) {
    ...
    const delivery = await deliverCompletedTimeline({
      ...
      deliveryEmailHash,
      result: tokenResult.result_json,
      pack: isPack ? { packToken, creditsRemaining: credits.creditsRemaining, product: maxPack ? 'max3' : 'pack3', expiresAt: packExpiry } : null,
    })
