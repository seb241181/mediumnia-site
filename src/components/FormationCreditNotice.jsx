...
// Découverte se connecte avec l'e-mail de cet achat et voit 368 € au lieu de 397 €.
export default function FormationCreditNotice() {
  ...
  if (state.status === 'credited') {
    return (
      <p role="status" className="mb-6 rounded-xl border border-gold/40 bg-gold/10 px-4 py-3 text-center font-georgia text-sm leading-relaxed text-deep">
        ...
        <strong>Vous réglez {euros(state.credit.displayAmount)}</strong> au lieu de 397 €.
      </p>
    )
  }
  ...
  if (state.status === 'parcours') {
    return (
      <p role="status" className="mb-6 rounded-xl border border-gold/40 bg-white/80 px-4 py-3 text-center font-georgia text-sm leading-relaxed text-deep">
        Vous suivez déjà le parcours au mois : terminez-le depuis <a href="/formation/parcours" className="text-gold underline">Mon parcours</a> avec « Tout débloquer », sans jamais dépasser 397 €.
      </p>
    )
  }
  ...
  return (
    <form onSubmit={submit} className="mb-6 rounded-xl border border-gold/25 bg-white/70 px-4 py-4 text-left">
      <p className="font-georgia text-sm text-deep">Vous avez déjà la Découverte ? Connectez-vous avec l’e-mail de cet achat : vos 29 € sont déduits (368 €).</p>
