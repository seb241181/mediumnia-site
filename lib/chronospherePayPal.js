...
function runtimeConfig() {
  ...
  if (isProduction) {
    if (process.env.PAYPAL_CHRONOSPHERE_ENABLED !== 'true') {
      ...
      const response = await fetch(`${cfg.base}/v2/checkout/orders`, {
        ...
        body: JSON.stringify({
          ...
          purchase_units: [{
            ...
            custom_id: `MEDIUMIA:${CONSENT_VERSION}:CHRONOSPHERE`,
          }],
          payment_source: {
  ...
  if (action === 'capture') {
    ...
    try {
      ...
      const { data: existing } = await supabase
        ...
        .select('id, draw_token_hash, status')
      ...
      if (existing) {
        return res.status(200).json({
          ...
          drawTokenReady: existing.status === 'ready',
        })
      }
