...
// founder, annual historical code. Temporary accesses (v2 codes, manual grants,
// unknown origins) never count here.
const PERMANENT_COMPLETE = [
  (r) => r.type === 'purchase' && /^paypal:(?:live|sandbox):[A-Za-z0-9_-]+$/.test(r.origin_ref || ''),
  (r) => r.type === 'purchase' && /^conference-pass:(?:live|sandbox):[A-Za-z0-9_-]+$/.test(r.origin_ref || ''),
...
async function hasCompleteAccess(supabase, userId) {
  const { data } = await supabase.from('mediumia_entitlements')
    .select('type, origin_ref, status, max_module, access_started_at, access_expires_at').eq('user_id', userId).eq('max_module', 25)
