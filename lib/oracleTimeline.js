...
async function callAnthropic(prompt) {
  ...
  if (!apiKey) {
    console.warn('[oracle-timeline] Anthropic: no API key found')
    return null
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const errorType = body.includes('"type"') ? (JSON.parse(body)?.error?.type || 'unknown') : 'unknown'
      console.error('[oracle-timeline] Anthropic failed: HTTP', response.status, 'type:', errorType)
      return null
    }
    ...
    console.error('[oracle-timeline] Anthropic network error:', err?.message || err)
    return null
  }
}
...
async function callOpenAI(prompt) {
  ...
  if (!apiKey) {
    console.warn('[oracle-timeline] OpenAI: no API key found')
    return null
  }
  ...
  try {
    ...
    if (!response.ok) {
      const body = await response.text().catch(() => '')
      const errorType = body.includes('"error"') ? (JSON.parse(body)?.error?.type || 'unknown') : 'unknown'
      console.error('[oracle-timeline] OpenAI failed: HTTP', response.status, 'type:', errorType)
      return null
    }
    ...
    console.error('[oracle-timeline] OpenAI network error:', err?.message || err)
    return null
  }
}
