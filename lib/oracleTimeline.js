...
- Présente la fenêtre prioritaire comme une période « astrologiquement plus porteuse », « plus fluide » ou « de moindre tension » selon les données, jamais comme « le meilleur moment garanti ».
...
REDIGE UNE LECTURE COHERENTE EN 6 PARTIES :
...
La réponse finale doit être immédiatement lisible par le client : uniquement les 6 parties demandées, sans préambule technique, sans notes internes et sans explication de ta méthode.`
}
...
export async function handleOracleTimeline(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      engine: 'chronosphere-999-58-v4',
      mode: 'cards-plus-natal-geometry-plus-timing-preview',
