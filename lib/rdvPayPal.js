...
export function hasCompetingRdvSlot({ currentHoldId, startsAt, endsAt, holds = [], bookings = [], now = Date.now() }) {
  const overlaps = item => new Date(item.starts_at).getTime() < new Date(endsAt).getTime()
    && new Date(item.ends_at).getTime() > new Date(startsAt).getTime()
