...
async function updateEmailDeliveryFailure(supabase, drawId, failureCode) {
...
async function deliverCompletedTimeline({ supabase, drawId, deliveryEmail, deliveryEmailHash, result }) {
  ...
  const emailResult = await sendChronosphereEmail({ drawId, deliveryEmail, result })
  if (emailResult.status !== 'sent') {
    await updateEmailDeliveryFailure(supabase, drawId, deliveryFailureCode(emailResult))
    return { email: 'error' }
  }
...
export async function handleOracleTimeline(req, res) {
  ...
  const tokenHash = createHash('sha256').update(drawToken).digest('hex')
  const requestHash = createHash('sha256')
    .update(JSON.stringify({ numbers, theme: normalizedTheme, profile: req.body?.profile, deliveryEmail }))
    .digest('hex')
  const deliveryEmailHash = createHash('sha256').update(deliveryEmail).digest('hex')

  const supabase = getSupabaseAdmin()
  const { data: tokenResult, error: tokenError } = await supabase.rpc('consume_chronosphere_draw_token', {
  ...
  if (tokenResult.cached && tokenResult.result_json) {
    ...
    return res.status(200).json({ ...tokenResult.result_json, delivery })
  }
  ...
  try {
    ...
    const { data: completeResult, error: completeError } = await supabase.rpc('complete_chronosphere_draw', {
      p_draw_id: drawId,
    ...
    return res.status(200).json({ ...responsePayload, delivery })
  } catch (error) {
    ...
    if (drawClaimedProcessing) {
      try {
        ...
        if (resetError) {
          console.error('[oracle-timeline] draw reset failed:', resetError.code || 'supabase_update_failed')
        }
      } catch {
