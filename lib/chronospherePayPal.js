...
const MAX_CONSENT_VERSION = 'chronosphere-max-2026-09-23-v1'
...
export async function handleChronospherePayPal(req, res, action) {
  ...
  if (action === 'status') {
    ...
    const { data: pack, error } = await supabase.from('chronosphere_credit_packs')
      .select('id, user_id, product_type, credits_remaining, credits_total, status').eq('pack_token_hash', tokenHash).maybeSingle()
  ...
  if (action === 'capture') {
    ...
    try {
      ...
      const isMax = pack.product_type === 'max3' || pack.consent_version === MAX_CONSENT_VERSION
      ...
      const expectedConsent = isMax ? MAX_CONSENT_VERSION : PACK_CONSENT_VERSION
