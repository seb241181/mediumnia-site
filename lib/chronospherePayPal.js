...
async function getAccessToken(cfg) {
  ...
  if (!response.ok) {
    ...
    console.error('[chronosphere-paypal] oauth failed:', {
      step: 'oauth',
...
function verifiedPayment(cfg, data) {
  ...
  // PayPal's capture/show-order responses may omit purchase_units[].custom_id.
  // If PayPal does echo a custom_id, it must match. Consent itself is verified
  // against the server-side chronosphere_paid_draws record before capture.
  const echoedCustomId = unit.custom_id || capture.custom_id || null
  if (echoedCustomId && echoedCustomId !== EXPECTED_CUSTOM_ID) {
...
export async function handleChronospherePayPal(req, res, action) {
  ...
  if (action === 'create') {
    ...
    try {
      ...
      if (!response.ok || !data.id) {
        console.error('[chronosphere-paypal] create order failed:', {
          step: 'create_order',
          status: response.status,
          name: data.name,
          message: data.message,
          debug_id: data.debug_id,
          details: (data.details || []).map(d => ({ issue: d.issue, description: d.description, field: d.field })).slice(0, 5),
        })
      ...
      console.error('[chronosphere-paypal] create action failed:', code)
  ...
  if (action === 'capture') {
    ...
    try {
      const supabase = getSupabaseAdmin()
      const { data: drawRecord, error: drawLookupError } = await supabase
        .from('chronosphere_paid_draws')
        .select('id, status, consent_version, consent_accepted_at, paypal_env, amount_cents, currency')
        .eq('paypal_order_id', orderId)
        .maybeSingle()

      if (drawLookupError) {
        console.error('[chronosphere-paypal] draw lookup failed:', drawLookupError.code)
        throw new Error('draw_lookup_failed')
      }
      if (!drawRecord) throw new Error('draw_not_found')
      if (drawRecord.consent_version !== CONSENT_VERSION || !drawRecord.consent_accepted_at) {
        throw new Error('paypal_consent_mismatch')
      }
      if (
        drawRecord.paypal_env !== cfg.env ||
        drawRecord.amount_cents !== cents(cfg.amount) ||
        drawRecord.currency !== cfg.currency
      ) {
        throw new Error('paypal_payment_invalid')
      }
