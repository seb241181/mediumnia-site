...
import { pathEnv } from './formationPath.js'
...
const PRODUCTS = {
  full: {
    ...
    live: {
      amount: '397.00',
      displayAmount: '397.00',
      referenceId: 'MEDIUMIA_FORMATION_397',
...
// (397 € → 368 €), en PayPal live uniquement. Jamais de montant venant du navigateur.
const DISCOVERY_CREDIT_CENTS = 2900
const CREDITED_FULL = {
  amount: '368.00',
  displayAmount: '368.00',
  referenceId: 'MEDIUMIA_FORMATION_368',
...
async function discoveryCredit(supabase, cfg, userId, getToken, { ownOrderId = null } = {}) {
  ...
  // A parcours already under way is completed with « Tout débloquer » (397 € − encaissé):
  // adding a 368 € purchase on top would exceed the 397 € cap.
...
async function provisionAccess(cfg, payment, intent = null) {
  ...
  try {
    ...
    // Parcours au mois: every Discovery / full purchase counts toward the 397 € cap.
    ...
    if (cfg.env === 'live') {
      ...
      const creditNoticeHtml = isDiscovery
        ? '<p><strong>Vos 29 € seront déduits</strong> si vous poursuivez ensuite avec la Formation complète : connectez-vous avec cette adresse e-mail sur mediumia.fr/formation, la Formation complète vous sera proposée à 368 €.</p>'
        : ''
      const creditNoticeText = isDiscovery
        ? '\n\nVos 29 € seront déduits si vous poursuivez ensuite avec la Formation complète : connectez-vous avec cette adresse e-mail sur mediumia.fr/formation, la Formation complète vous sera proposée à 368 €.'
        : ''
...
async function recoverOrCapture(baseCfg, orderId) {
  ...
  if (intent.upgrade_credit_purchase_id) {
    if (intent.amount_cents !== 36800 || !intent.user_id) throw new Error('purchase_product_mismatch')
