...
export default function ProWaitlistPublic({ onBack, onNavigate }) {
  ...
  return (
    <div className="cosmic-page cosmic-page--agents bg-cream min-h-screen text-deep">
        ...
        <section className="px-6 py-16 max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {CAPABILITIES.map(cap => (
              <article key={cap.title} className="rounded-2xl border border-gold/20 p-7 bg-white/55 shadow-sm flex flex-col">
            ...
            {success ? (
              ...
              <form onSubmit={handleSubmit} className="rounded-2xl border border-gold/25 bg-white/70 p-7 md:p-10 shadow-sm space-y-5">
                <div>
                  ...
                  <input type="text" value={form.firstName} onChange={e => setForm(f => ({ ...f, firstName: e.target.value.slice(0, 80) }))} className="w-full font-georgia text-sm border border-gold/25 rounded-lg px-4 py-3 bg-white focus:outline-none focus:border-gold/60 transition-colors" required />
                </div>

                <div>
                  <label className="font-georgia text-sm font-medium text-deep block mb-1.5">Email *</label>
                  <input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value.slice(0, 254) }))} className="w-full font-georgia text-sm border border-gold/25 rounded-lg px-4 py-3 bg-white focus:outline-none focus:border-gold/60 transition-colors" required />
                </div>

                <div>
                  <label className="font-georgia text-sm font-medium text-deep block mb-1.5">Activité / métier *</label>
                  <input type="text" value={form.activity} onChange={e => setForm(f => ({ ...f, activity: e.target.value.slice(0, 120) }))} className="w-full font-georgia text-sm border border-gold/25 rounded-lg px-4 py-3 bg-white focus:outline-none focus:border-gold/60 transition-colors" placeholder="Ex : Médium, praticien bien-être, formateur…" required />
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {NEEDS.map(need => (
                      <button key={need} type="button" onClick={() => setForm(f => ({ ...f, primaryNeed: need }))} className="font-georgia text-sm text-left px-4 py-3 rounded-lg border transition-all" style={form.primaryNeed === need ? { background: '#1A1535', color: '#C9A84C', borderColor: '#1A1535' } : { background: 'white', color: '#4A3F6B', borderColor: 'rgba(201,168,76,.25)' }}>
                        {need}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  <label className="font-georgia text-sm font-medium text-deep block mb-1.5">Qu'est-ce qui vous ferait gagner le plus de temps ? <span className="text-mist font-normal">(facultatif)</span></label>
                  <textarea value={form.message} onChange={e => setForm(f => ({ ...f, message: e.target.value.slice(0, 1000) }))} rows={3} className="w-full font-georgia text-sm border border-gold/25 rounded-lg px-4 py-3 bg-white focus:outline-none focus:border-gold/60 transition-colors resize-none" />
                  <p className="font-georgia text-[10px] text-mist/50 text-right mt-1">{form.message.length}/1000</p>
                </div>

                <label className="flex items-start gap-3 cursor-pointer">
                  <input type="checkbox" checked={form.consent} onChange={e => setForm(f => ({ ...f, consent: e.target.checked }))} className="mt-1 shrink-0 accent-gold" />
                ...
                <button type="submit" disabled={!canSubmit} className="w-full font-georgia px-8 py-4 rounded-lg bg-gold text-deep font-bold text-base hover:bg-gold/90 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
