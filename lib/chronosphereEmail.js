...
import { getChronosphereReading, readingToInterpretation } from './chronosphereReading.js'
...
// Confirmation sur support durable (art. L. 221-13) de l'accord donné avant paiement.
const CONSENT_REMINDER = 'Avant le paiement, vous avez demandé l’exécution immédiate de ce contenu numérique personnalisé et reconnu perdre votre droit de rétractation une fois la lecture générée (art. L. 221-28, 13° du Code de la consommation).'

// CGV : tirages et lectures valables 6 mois (packs achetés à partir du 24/09/2026).
function packValidityText(expiresAt) {
  const date = expiresAt ? new Date(expiresAt) : null
  if (!date || !Number.isFinite(date.getTime())) return ''
  const label = new Intl.DateTimeFormat('fr-FR', { day: 'numeric', month: 'long', year: 'numeric', timeZone: 'Europe/Paris' }).format(date)
  return ` À utiliser avant le ${label}.`
}

function buildPackEmailSection(pack) {
  const creditsRemaining = Number(pack?.creditsRemaining)
  const validity = packValidityText(pack?.expiresAt)
  if (!Number.isInteger(creditsRemaining)) return { html: '', text: '' }

  if (pack?.product === 'max3') {
    const maxUrl = 'https://mediumia.fr/chronosphere-max'
    if (creditsRemaining === 1 || creditsRemaining === 2) {
      const remainingText = creditsRemaining === 1
        ? 'Il vous reste 1 lecture ChronoSphère MAX dans cette Ligne de Temps.'
        : 'Il vous reste 2 lectures ChronoSphère MAX dans cette Ligne de Temps.'
      return {
        html: `<div style="margin-top:28px;padding:22px 20px;border:1px solid #e1d2aa;border-radius:16px;background:#faf6ea;text-align:center;">
            <p style="margin:0 0 8px;color:#a98536;font-size:11px;letter-spacing:1.5px;text-transform:uppercase;">Votre suivi ChronoSphère MAX</p>
            <p style="margin:0 0 18px;color:#1a1535;font-size:15px;line-height:1.6;">${remainingText}${validity}</p>
            <a href="${maxUrl}" style="display:inline-block;padding:13px 20px;border-radius:10px;background:#1a1535;color:#f8f5ee;font-size:14px;text-decoration:none;">Continuer ma Ligne de Temps</a>
            <p style="margin:14px 0 0;color:#756d7e;font-size:11px;line-height:1.5;">Connectez-vous avec le même compte MediumIA pour retrouver la mémoire de vos lectures.</p>
          </div>`,
        text: `\n\nVOTRE SUIVI CHRONOSPHÈRE MAX\n${remainingText}${validity}\nContinuer ma Ligne de Temps : ${maxUrl}\nConnectez-vous avec le même compte MediumIA.`,
      }
    }
    ...
    if (creditsRemaining === 0) {
      return {
        html: `<div style="margin-top:28px;padding:22px 20px;border:1px solid #e1d2aa;border-radius:16px;background:#faf6ea;text-align:center;">
            <p style="margin:0 0 8px;color:#a98536;font-size:11px;letter-spacing:1.5px;text-transform:uppercase;">Votre suivi ChronoSphère MAX</p>
            <p style="margin:0 0 18px;color:#1a1535;font-size:15px;line-height:1.6;">Vos 3 lectures MAX sont maintenant réunies. Votre synthèse finale reste accessible depuis votre compte MediumIA.</p>
            <a href="${maxUrl}" style="display:inline-block;padding:13px 20px;border-radius:10px;background:#1a1535;color:#f8f5ee;font-size:14px;text-decoration:none;">Retrouver ma Ligne de Temps</a>
          </div>`,
        text: `\n\nVOTRE SUIVI CHRONOSPHÈRE MAX\nVos 3 lectures MAX sont maintenant réunies.\nRetrouver ma Ligne de Temps : ${maxUrl}`,
      }
    }
...
function buildReadingEmailSection(result) {
  const reading = getChronosphereReading(result)
  if (!reading.summary30s && !reading.sections.length) {
    const fallback = String(result?.interpretation || '')
    return {
      html: `<div style="padding:22px 0 4px;border-top:1px solid #e9e1d0;color:#2f2940;font-size:15px;line-height:1.9;white-space:pre-line;">${escapeHtml(fallback)}</div>`,
      text: fallback,
    }
  }

  const directionHtml = reading.direction?.label
    ? `<div style="margin:18px 0;padding:18px;border:1px solid #d4b469;border-radius:14px;background:#faf6ea;">
        <p style="margin:0 0 8px;color:#a98536;font-size:11px;letter-spacing:1.5px;text-transform:uppercase;">Tendance du tirage</p>
        <p style="margin:0;color:#1a1535;font-size:18px;line-height:1.45;"><strong>${escapeHtml(reading.direction.label)}</strong></p>
        ${reading.direction.content ? `<p style="margin:8px 0 0;color:#5f5870;font-size:14px;line-height:1.7;">${escapeHtml(reading.direction.content)}</p>` : ''}
      </div>`
    : ''
  const sectionHtml = reading.sections.map((section) => (
    `<article style="margin:18px 0;padding:18px 0 0;border-top:1px solid #e9e1d0;">
      <p style="margin:0 0 6px;color:#a98536;font-size:11px;letter-spacing:1.4px;text-transform:uppercase;">${String(section.number).padStart(2, '0')}</p>
      <h2 style="margin:0 0 10px;color:#1a1535;font-size:21px;font-weight:normal;line-height:1.3;">${escapeHtml(section.title)}</h2>
      <div style="color:#2f2940;font-size:15px;line-height:1.9;white-space:pre-line;">${escapeHtml(section.content)}</div>
    </article>`
  )).join('')

  return {
    html: `<div style="padding:22px 0 4px;border-top:1px solid #e9e1d0;">
      <div style="margin:0 0 18px;padding:18px;border-radius:14px;background:#f8f5ee;">
        <p style="margin:0 0 8px;color:#a98536;font-size:11px;letter-spacing:1.5px;text-transform:uppercase;">Votre tirage en 30 secondes</p>
        <p style="margin:0;color:#1a1535;font-size:15px;line-height:1.8;">${escapeHtml(reading.summary30s)}</p>
      </div>
      ${directionHtml}
      ${sectionHtml}
    </div>`,
    text: readingToInterpretation(reading),
  }
}
...
export function buildChronosphereEmail(result, pack = null) {
  ...
  const readingEmail = buildReadingEmailSection(result)
