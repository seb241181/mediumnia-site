...
test('cosmic motion is CSS-only, responsive and disabled for reduced motion', async () => {
  ...
  assert.doesNotMatch(styles, /cosmic-blink|cosmic-library__eye-blink|cosmic-library__eyelid|cosmic-library__blink-seam/)
  assert.match(styles, /\.cosmic-dock__detail \{[\s\S]*display: -webkit-box;[\s\S]*-webkit-line-clamp: 2;/)
