...
import { pathOpen, settlePathAfterFullPurchase } from './formationPath.js'
...
// TEMPORAIRE (diagnostic Sandbox, préversion uniquement) : étape en échec et réponse
// brute de PayPal / Supabase (statut, nom d'erreur, debug_id, code). Jamais
// d'identifiant, de secret, de jeton ni de donnée client. À retirer après le diagnostic.
function diagError(code, diag) {
  const error = new Error(code)
  error.diag = diag
  return error
}
function logCheckoutDiag(action, error) {
  if (process.env.VERCEL_ENV === 'production' || !error?.diag) return
  console.error(`[paypal][diag] ${action} failed:`, JSON.stringify({ code: error.message, ...error.diag }))
}
...
async function getAccessToken(cfg) {
  ...
  if (!response.ok || !data.access_token) {
    throw diagError('paypal_auth_failed', { step: 'auth', env: cfg.env, http: response.status, paypal_error: data?.error || null, paypal_error_description: data?.error_description || null })
  }
...
async function saveOrderIntent(supabase, cfg, orderId, credit = null, acceptedAt = new Date().toISOString()) {
  ...
  if (error) throw diagError('order_intent_write_failed', { step: 'intent', env: cfg.env, product: cfg.product, db_code: error.code || null, db_message: error.message || null, db_hint: error.hint || null })
}
...
async function provisionAccess(cfg, payment, intent = null) {
  ...
  try {
    ...
    if (cfg.product === 'discovery' && pathOpen()) {
  ...
  if (action === 'create') {
    ...
    try {
      ...
      if (!response.ok || !data.id) {
        throw diagError('paypal_create_order_failed', { step: 'orders', env: cfg.env, product: cfg.product, http: response.status, paypal_name: data?.name || null, paypal_message: data?.message || null, debug_id: data?.debug_id || null, issues: (data?.details || []).map((d) => d.issue).filter(Boolean) })
      }
