...
create table public.pro_document_versions (
  ...
  constraint pro_document_versions_approval_check check (
    not approved_for_ai
    or (extraction_status = 'ready' and approved_at is not null)
  ),
  constraint pro_document_versions_publication_check check (
    published_at is null
    or (extraction_status = 'ready' and approved_for_ai and approved_at is not null)
  )
);
...
create unique index pro_audit_events_request_uidx
  on public.pro_audit_events(workspace_id, event_type, request_id)
...
begin
  ...
  on conflict (workspace_id, event_type, request_id)
    where request_id is not null
  do nothing;
end;
$$;
...
begin
  ...
  select v.document_id, v.id, v.storage_path, a.agent_id, d.lifecycle_status
    into v_document_id, v_version_id, v_storage_path, v_existing_agent_id, v_existing_lifecycle_status
  from public.pro_document_versions v
  ...
  if v_document_id is not null then
    if v_existing_agent_id <> p_agent_id then
  ...
  v_extension := lower(substring(trim(p_name) from '\.([a-z0-9]+)$'));
  ...
  insert into public.pro_documents(
    ...
    v_document_id, v_workspace_id, trim(p_name), p_actor_user_id
  );
...
begin
  ...
  if not v_has_current then
    ...
    set status = 'error',
        approved_for_ai = false,
        approved_at = null,
        error_message = trim(p_error_code)
    where d.id = v_document_id
      and d.workspace_id = v_workspace_id;
...
begin
  ...
  select q.dirty_revision into v_queue_revision
  ...
  if v_document.id is not null then
    raise exception using errcode = 'P0001', message = 'legacy_shadow_divergence';
  end if;
  ...
  insert into public.pro_document_versions(
    ...
    v_approved,
    case when v_approved then coalesce(v_legacy.approved_at, v_legacy.updated_at, v_legacy.created_at) else null end,
    case when v_approved then v_created_by else null end,
    null,
  ...
  if v_approved then
    update public.pro_document_versions v
    set published_at = coalesce(v_legacy.approved_at, v_legacy.updated_at, v_legacy.created_at),
        state_revision = v.state_revision + 1
    where v.id = v_version_id;
