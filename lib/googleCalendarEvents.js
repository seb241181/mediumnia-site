  ...
  try {
    ...
    if (calRes.ok) {
      ...
      await supabase
        ...
        .eq('id', bookingId)
        .eq('practitioner_id', practitionerId)
      return { status: 'synced', google_event_id: calData.id }
    }
    ...
    if (calRes.status === 409) {
      ...
      if (getRes.ok) {
        ...
        await supabase
          ...
          .eq('id', bookingId)
          .eq('practitioner_id', practitionerId)
        return { status: 'synced', google_event_id: existing.id }
      }
    }

    console.error(`[googleCalendarEvents] Google HTTP ${calRes.status} pour booking ${bookingId}`)
    return { status: 'failed', google_event_id: null }
  } catch (err) {
