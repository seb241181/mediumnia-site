  ...
  if (action === 'status') {
    ...
    if (packToken) {
      ...
      const lookup = await supabase.from('chronosphere_credit_packs')
        .select('id, user_id, product_type, credits_remaining, credits_total, status, created_at, captured_at')
    ...
    return res.status(200).json({
      ...
      // An expired pack reads as used up: its remaining draws are no longer available.
      creditsRemaining: expired ? 0 : pack.credits_remaining,
      expired,
      expiresAt: packExpiresAt(pack),
