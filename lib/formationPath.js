...
// Students who already hold the complete formation another way (conference
// pass, former access code, founder) are complete: nothing is ever offered or
// charged to them, and they lose nothing.
async function hasCompleteAccess(supabase, userId) {
  const { data } = await supabase.from('mediumia_entitlements')
    .select('origin_ref, status, max_module').eq('user_id', userId).eq('max_module', 25)
  return (data || []).some((r) => r.status !== 'revoked' && !String(r.origin_ref || '').startsWith('parcours:'))
}
