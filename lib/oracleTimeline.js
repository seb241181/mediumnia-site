...
- Les positions planétaires, angles, maisons et aspects fournis sont CALCULES côté serveur. Leur interprétation reste symbolique et ne doit jamais être présentée comme une causalité scientifiquement démontrée.
...
- N'invente JAMAIS une planète, un signe, un degré, un aspect, un transit, une maison, un Ascendant ou un angle absent des données calculées.
...
export async function handleOracleTimeline(req, res) {
  ...
  try {
    ...
    const astrology = calculateOracleAstrology(profile, location, drawDate)
    ...
    return res.status(200).json({
      engine: 'chronosphere-999-58-v3',
      mode: 'cards-plus-natal-geometry-preview',
