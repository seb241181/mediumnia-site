...
export function buildRdvPaymentConfirmation({
  firstName,
  serviceTitle,
  startsAt,
  ...
  const paidInFull = paymentOption === 'full' || balanceCents === 0
  const subject = 'MediumIA — Votre rendez-vous est confirmé'
  const intro = paidInFull
    ? 'Votre paiement intégral a bien été reçu. Votre créneau est maintenant confirmé.'
    : 'Vos arrhes de réservation ont bien été réglées. Votre créneau est maintenant confirmé.'
  const paidLabel = paidInFull ? 'Montant réglé' : 'Arrhes réglées'
  const legalTail = paidInFull
    ? 'À moins de 48 heures, l’annulation automatique est bloquée. Les conditions de réservation acceptées et les droits légaux applicables restent applicables.'
...
<div style="max-width:580px;margin:0 auto;padding:40px 24px;">
  ...
  <div style="background:#F0EDE8;border-radius:12px;padding:20px 24px;margin:22px 0;">
    ...
    <p style="margin:4px 0;font-size:14px;color:#4A3F6B;"><strong>${paidLabel} :</strong> ${money(paidCents)}</p>
  ...
  <p style="font-size:12px;line-height:1.6;color:#756B89;">${escapeHtml(legalTail)}</p>
