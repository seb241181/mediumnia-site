(() => {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .history-item button[disabled]{opacity:.55;cursor:not-allowed}.history-access{display:block;margin-top:4px;color:#c9a84c;font-size:10px}.follow-locked-notice{display:none;margin:12px 0 0;padding:11px 13px;border-left:3px solid #c9a84c;background:rgba(201,168,76,.08);border-radius:0 10px 10px 0;color:#5f5968;font-size:11px;line-height:1.5}.follow-locked-notice.visible{display:block}
  ...
  async function fetchServerPlan(lineId) {
    ...
    try {
      ...
      return { ...cached, plan, followToken: cached.followToken }
    } catch (error) {
  ...
  function gateHistoryButtons() {
    document.querySelectorAll('#historyList [data-resume]').forEach(button => {
      ...
      button.disabled = !access.enabled
      button.textContent = access.enabled ? `Revenir · ${access.text.match(/\d+/)?.[0] || ''}/${MAX_RETURNS}` : access.text
  ...
  function lockFollowFields(locked) {
    const ids = ['fullName', 'birthDate', 'birthTime', 'birthPlace', 'theme', 'customTheme']
    ids.forEach(id => {
      const element = document.getElementById(id)
      if (element) element.disabled = Boolean(locked)
    })

    const noticeHost = document.getElementById('resumeNotice')
    if (noticeHost) {
      noticeHost.classList.toggle('follow-locked-notice', locked)
      if (locked) {
        noticeHost.classList.add('visible')
        noticeHost.insertAdjacentHTML('beforeend', '<br><strong>Suivi actif :</strong> cette étape reprend exactement la même ligne. Seuls les trois nombres sont à choisir de nouveau.')
      }
    }
  }
  ...
  function observeSuccessfulReading() {
    ...
    const observer = new MutationObserver(() => {
      if (!followSubmissionArmed) return
      if (!reading.textContent.trim()) return
  ...
  function showOfferWhenReady() {
    ...
    pricingPanel.classList.toggle('visible', visible)
    if (!visible) return

    const latest = loadHistory()[0]
    const lineId = lineIdFor(latest)
    const cached = lineId ? planCache(lineId) : null
    if (cached?.followToken) renderActivePlan(lineId, cached.plan)
  }
