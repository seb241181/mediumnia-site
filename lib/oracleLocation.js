...
export async function resolveBirthLocation(placeValue) {
  ...
  let timeZones = []
  try {
    timeZones = findTimeZones(latitude, longitude)
  } catch {
    throw new Error('birth_timezone_unavailable')
  }

  const timeZone = Array.isArray(timeZones) ? timeZones[0] : null
  if (!timeZone) throw new Error('birth_timezone_unavailable')
