...
test('CODEX is public in the MediumIA boutique with its own cover and Amazon link', () => {
  ...
  assert.match(products, /salesCount: 35/)
...
test('CODEX sales milestone is visible on the boutique card', () => {
  const ecommerce = read('src/components/BoutiqueEcommerce.jsx')
  assert.match(ecommerce, /product\.salesCount/)
  assert.match(ecommerce, /exemplaires vendus/)
})
