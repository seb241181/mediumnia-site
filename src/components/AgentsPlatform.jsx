...
const templates = [
  { id: 'commerce', icon: '◈', category: 'Commerce', name: 'Assistant Boutique', description: 'Répond aux questions clients, présente vos produits, explique vos horaires et oriente vers l’achat.', tag: 'Prêt à personnaliser', starter: 'Je tiens une boutique et je veux un assistant qui répond aux questions de mes clients, présente mes produits et les aide avant leur achat.' },
  { id: 'cabinet', icon: '✦', category: 'Bien-être', name: 'Assistant Cabinet', description: 'Présente vos accompagnements, répond aux questions fréquentes et prépare la prise de rendez-vous.', tag: 'Prêt à personnaliser', starter: 'Je travaille dans le bien-être et je veux un assistant qui présente mes accompagnements, répond aux questions et prépare les rendez-vous.' },
  { id: 'formation', icon: '◇', category: 'Formation', name: 'Assistant Formation', description: 'Accompagne vos élèves à partir de vos contenus, méthodes, règles et documents.', tag: 'Mémoire métier', starter: 'Je propose une formation et je veux un assistant qui accompagne mes élèves à partir de mes contenus et de ma méthode.' },
  { id: 'tourisme', icon: '⌖', category: 'Tourisme', name: 'Assistant Tourisme', description: 'Informe les visiteurs, recommande des activités et répond à partir de vos informations locales.', tag: 'Multicanal', starter: 'Je travaille dans le tourisme et je veux un assistant qui informe les visiteurs et recommande les bonnes activités.' },
  { id: 'immobilier', icon: '⌂', category: 'Immobilier', name: 'Assistant Immobilier', description: 'Qualifie les demandes, présente vos biens et prépare les informations utiles avant un échange humain.', tag: 'Professionnel', starter: 'Je travaille dans l’immobilier et je veux un assistant qui qualifie les prospects, présente les biens et prépare mes rendez-vous.' },
  { id: 'social', icon: '✺', category: 'Marketing', name: 'Assistant Réseaux', description: 'Aide à préparer publications, idées de contenus et réponses cohérentes avec votre identité.', tag: 'Création', starter: 'Je veux un assistant qui m’aide à préparer mes publications et à garder une communication cohérente sur mes réseaux.' },
]

const categories = ['Tous', ...new Set(templates.map((item) => item.category))]
const questions = [
  { key: 'mission', label: 'Que voulez-vous confier à votre assistant ?', placeholder: 'Décrivez votre activité et ce que vous aimeriez que votre assistant fasse pour vous…' },
  { key: 'audience', label: 'Avec qui votre assistant va-t-il principalement parler ?', placeholder: 'Exemple : mes clients, mes prospects, mes élèves, les visiteurs de mon site…' },
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4" onClick={onClose}>
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-deep text-cream p-8 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="font-georgia text-cream/55 text-sm text-center mb-6">MediumIA · Assistants IA</p>
  ...
  return (
    <section className="px-6 pb-24 max-w-4xl mx-auto">
      <div className="flex items-center justify-between gap-4 mb-8">
        <h2 className="font-georgia text-3xl text-deep">Mes assistants</h2>
        <button onClick={onCreate} className="font-georgia text-sm px-5 py-2.5 rounded-lg bg-deep text-gold font-bold">+ Nouvel assistant</button>
      </div>

      {status === 'loading' && <p className="font-georgia text-mist text-center py-16">Chargement de vos assistants…</p>}

      {status === 'error' && (
        <div className="rounded-2xl border border-red-300/40 bg-red-50 p-6 text-center">
          <p className="font-georgia text-red-500 mb-1">Impossible de charger vos assistants.</p>
      ...
      {status === 'ready' && agents.length === 0 && (
        <div className="rounded-3xl border border-gold/25 bg-white/55 p-10 text-center">
          ...
          <p className="font-georgia text-deep text-xl mb-2">Aucun assistant pour l’instant.</p>
          <p className="font-georgia text-mist mb-6">Créez votre premier assistant : MediumIA le construit à partir de votre métier.</p>
          <button onClick={onCreate} className="font-georgia px-8 py-4 rounded-lg bg-gold text-deep font-bold">Créer mon premier assistant</button>
        </div>
      )}
        ...
        <div className="grid md:grid-cols-2 gap-5">
          {agents.map((agent) => (
            <article key={agent.id} className="rounded-2xl border border-gold/25 bg-white/55 p-6 shadow-sm flex flex-col">
              ...
              <p className="font-georgia text-mist text-sm leading-relaxed line-clamp-4 flex-1">{agent.mission || 'Assistant en préparation.'}</p>
...
function AgentCreator({ initialMission = '', user, onRequireAuth, onCreated }) {
  ...
  if (finished) {
    return <div className="rounded-3xl bg-deep text-cream p-7 md:p-10 shadow-xl">
      ...
      <h2 className="font-georgia text-3xl md:text-4xl mb-4">Votre assistant commence à prendre forme.</h2>
      ...
      {saved ? (
        ...
        <>
          <div className="mt-7 rounded-xl border border-gold/30 bg-gold/10 p-5"><p className="font-georgia text-sm text-gold font-bold mb-2">Prochaine étape</p><p className="font-georgia text-sm text-cream/70 leading-relaxed">Enregistrer cet assistant dans votre espace personnel. Il restera en brouillon, prêt à être affiné puis activé quand vous le déciderez.</p></div>
          {saveError && <p className="font-georgia text-sm text-red-300 mt-4">{saveError}</p>}
          <div className="mt-6 flex flex-col sm:flex-row items-center gap-4">
            <button onClick={createAgent} disabled={saving} className="font-georgia px-8 py-4 rounded-lg bg-gold text-deep font-bold disabled:opacity-50">{saving ? 'Enregistrement…' : (user ? 'Créer mon assistant' : 'Se connecter pour créer mon assistant')}</button>
...
export default function AgentsPlatform({ onBack, onOpenReseau }) {
  ...
  return <div className="min-h-screen bg-cream text-deep">
    <header className="sticky top-0 z-50 bg-cream/95 backdrop-blur-sm border-b border-gold/20"><div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between gap-4"><button onClick={onBack} className="font-georgia text-deep tracking-[0.18em] text-sm md:text-base font-semibold">✦ MEDIUMIA</button><div className="flex items-center gap-2 md:gap-3">
      ...
      {user && <button onClick={openMyAgents} className={`font-georgia text-xs md:text-sm px-3 py-2 rounded-lg ${tab === 'mes-agents' || tab === 'chat' ? 'bg-deep text-gold' : 'text-mist'}`}>Mes assistants</button>}
      {user ? (
      ...
      {tab === 'explorer' && <section className="px-6 pb-20 max-w-6xl mx-auto"><div className="flex gap-2 overflow-x-auto pb-5">{categories.map((item) => <button key={item} onClick={() => setCategory(item)} className={`shrink-0 font-georgia text-sm px-4 py-2 rounded-full border ${category === item ? 'bg-deep text-gold border-deep' : 'border-gold/30 text-mist'}`}>{item}</button>)}</div><div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">{visible.map((agent) => <article key={agent.id} className="rounded-2xl border border-gold/25 bg-white/55 p-6 flex flex-col min-h-[300px] shadow-sm"><div className="flex items-start justify-between mb-6"><span className="text-gold text-3xl">{agent.icon}</span><span className="font-georgia text-[11px] uppercase tracking-wider text-mist bg-deep/5 rounded-full px-3 py-1">{agent.category}</span></div><h2 className="font-georgia text-2xl mb-3">{agent.name}</h2><p className="font-georgia text-mist text-sm leading-relaxed flex-1">{agent.description}</p><div className="mt-6 pt-5 border-t border-gold/15 flex items-center justify-between gap-3"><span className="font-georgia text-xs text-gold">{agent.tag}</span><button onClick={() => openCreator(agent.starter)} className="font-georgia text-sm font-bold">Personnaliser →</button></div></article>)}</div>
        <div className="mt-20 border-t-2 border-gold/20 pt-16 pb-4">
          <div className="max-w-2xl mx-auto text-center">
            ...
            <h2 className="font-georgia font-medium text-3xl text-deep leading-tight mb-4">Rejoindre le réseau MediumIA</h2>
            <p className="font-georgia text-mist text-sm leading-relaxed mb-2 max-w-xl mx-auto">Faites découvrir votre pratique à celles et ceux qui cherchent un accompagnement qui leur correspond.</p>
            <p className="font-georgia text-mist text-sm leading-relaxed mb-8 max-w-xl mx-auto">MediumIA construit progressivement un réseau de praticiens du spirituel, du bien-être et de la transmission. Chaque demande est examinée avant publication afin de préserver la cohérence et la qualité du réseau.</p>
            <button onClick={onOpenReseau} className="font-georgia px-8 py-4 rounded-xl border-2 border-gold text-deep font-bold hover:bg-gold/10 transition-colors">Faire une demande d'inscription →</button>
          </div>
        </div>
      </section>}
