 ...
 *   POST subscribe       crée l'abonnement PayPal (34 € répétés puis échéance finale) → id pour les boutons
 ...
 * jamais dépasser 397 €. En production, fermé tant que
 * PAYPAL_FORMATION_PATH_ENABLED !== 'true'. Aucune donnée personnelle dans les logs.
 */
...
import { CAP_CENTS, STEP_CENTS, TOTAL_MODULES, euros, exceedsCap, nextStepModules, scheduleFor, summarize } from './formationProgression.js'

export const PATH_TERMS_VERSION = 'formation-parcours-2026-09-25-v1'
const PLAN_CODE = 'parcours-34x-final'
const PAYPAL = {
...
export function pathEnv() {
  ...
  if (isProduction && process.env.PAYPAL_FORMATION_PATH_ENABLED !== 'true') return null
...
// Plan: 10 × 34 € (TRIAL cycles) then 1 × 28 € (REGULAR); each subscription
...
export async function ensurePlan(supabase, env, access) {
  ...
  const plan = await paypal(env, access, 'POST', '/v1/billing/plans', {
    ...
    name: 'MediumIA — Parcours au mois (397 € maximum)',
    status: 'ACTIVE',
    billing_cycles: [
      { frequency: { interval_unit: 'MONTH', interval_count: 1 }, tenure_type: 'TRIAL', sequence: 1, total_cycles: 10, pricing_scheme: { fixed_price: { value: value2(STEP_CENTS), currency_code: 'EUR' } } },
      { frequency: { interval_unit: 'MONTH', interval_count: 1 }, tenure_type: 'REGULAR', sequence: 2, total_cycles: 1, pricing_scheme: { fixed_price: { value: '28.00', currency_code: 'EUR' } } },
    ],
...
// founder, annual historical code, the historical legacy-paypal purchase.
...
const LEGACY_PAYPAL_CUTOFF = new Date('2026-09-25T00:00:00Z').getTime()
const PERMANENT_COMPLETE = [
  (r) => r.type === 'purchase' && /^paypal:live:[A-Za-z0-9_-]+$/.test(r.origin_ref || ''),
  // The one historical complete purchase taken over by hand (597 € of 13/08/2026).
  (r) => r.type === 'purchase' && /^legacy-paypal:[A-Za-z0-9_-]+$/.test(r.origin_ref || '') && r.access_level === 'full'
    && r.status === 'active' && new Date(r.access_started_at || 0).getTime() < LEGACY_PAYPAL_CUTOFF,
  (r) => r.type === 'purchase' && /^conference-pass:live:[A-Za-z0-9_-]+$/.test(r.origin_ref || ''),
  (r) => r.type === 'admin' && /^founder:[0-9a-f-]{36}$/i.test(r.origin_ref || ''),
  (r) => r.type === 'legacy_code' && (new Date(r.access_expires_at) - new Date(r.access_started_at)) / 86_400_000 >= 365 - 1 / 24,
]
...
export async function syncSubscription(supabase, env, subscriptionId, access) {
  ...
  const before = summarize(await loadPayments(supabase, row.user_id, env))
  ...
  const after = summarize(await loadPayments(supabase, row.user_id, env))
  ...
  // Guard: never a cent above 397 €. A live subscription stops as soon as the
  ...
  if ((after.complete || overCap) && ['active', 'approval_pending'].includes(status)) {
    await paypal(env, access, 'POST', `/v1/billing/subscriptions/${encodeURIComponent(subscriptionId)}/cancel`, { reason: 'Parcours MediumIA complet (397 €)' })
    status = after.complete ? 'completed' : 'cancelled'
  }
  if (overCap) await warnOwner('Parcours : paiement au-delà de 397 €', `Un abonnement du parcours a encaissé au-delà du plafond (abonnement ${subscriptionId}). Vérifiez-le dans PayPal et remboursez l'excédent.`)
...
export async function syncLiveSubscriptions(supabase) {
  const env = pathEnv()
  if (!env) return { skipped: 'disabled' }
...
export async function handleFormationPath(req, res, action, supabaseForTests = null) {
  ...
  if (action === 'webhook') {
    ...
    if (!env) return res.status(200).json({ ok: true, ignored: 'disabled' })
  ...
  try {
    const payments = await loadPayments(supabase, user.id, env)
    ...
    if (action === 'status') {
      ...
      return res.status(200).json({ env, clientId: String(process.env.PAYPAL_CLIENT_ID || '').trim() || null, termsVersion: PATH_TERMS_VERSION, ...publicState(summary, sub) })
    }
    ...
    if (action === 'subscribe') {
      ...
      if (!summary.hasDiscovery && !summary.hasFull) return res.status(409).json({ error: 'discovery_required' })
    ...
    if (action === 'activate') {
      ...
      const result = await syncSubscription(supabase, env, subscriptionId, await token(env))
      const fresh = summarize(await loadPayments(supabase, user.id, env))
    ...
    if (action === 'unlock-capture') {
      ...
      const current = summarize(await loadPayments(supabase, user.id, env))
      // Capture only if the order still fits under 397 €, otherwise nothing is charged.
      if (exceedsCap(current, order.amount_cents)) {
      ...
      const after = summarize(await loadPayments(supabase, user.id, env))
    ...
    if (action === 'cancel') {
      ...
      if (env === 'live' && user.email) {
        const text = `Votre parcours MediumIA est arrêté : aucun nouveau prélèvement ne sera fait.\n\nVos modules débloqués (jusqu’au module ${summary.maxModule}), vos PDF et votre carnet restent à vous. Vous pouvez reprendre quand vous voulez : ${SITE}/formation/parcours\n\nSébastien · MediumIA`
