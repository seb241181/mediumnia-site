...
export async function handleOracleTimeline(req, res) {
  ...
  const maxAccount = req.body?.maxAccount === true
  if (!packToken && !drawToken && !maxAccount) {
    return res.status(402).json({ error: 'payment_required', message: 'Un tirage payant est requis.' })
  }

  const isPack = Boolean(packToken || maxAccount)
  const drawTable = isPack ? 'chronosphere_pack_draws' : 'chronosphere_paid_draws'
  const supabase = getSupabaseAdmin()

  let tokenHash = drawToken ? createHash('sha256').update(drawToken).digest('hex') : ''
  let maxPack = null
  let maxUser = null
  ...
  if (maxAccount) {
    ...
    if (!maxUser) return res.status(401).json({ error: 'auth_required', message: 'Connectez-vous au compte MediumIA ayant acheté ChronoSphère MAX.' })

    let lookup = await supabase.from('chronosphere_credit_packs')
      .select('id, user_id, product_type, pack_token_hash, status')
      .eq('user_id', maxUser.id)
    ...
    if (!lookup.data && !lookup.error) {
      lookup = await supabase.from('chronosphere_credit_packs')
        .select('id, user_id, product_type, pack_token_hash, status')
        .eq('user_id', maxUser.id)
        .eq('product_type', 'max3')
        .eq('status', 'exhausted')
    ...
    if (lookup.error) return res.status(503).json({ error: 'pack_status_unavailable' })
    if (!lookup.data) return res.status(402).json({ error: 'payment_required', message: 'Aucun suivi ChronoSphère MAX actif n’est rattaché à ce compte.' })
    maxPack = lookup.data
    tokenHash = maxPack.pack_token_hash
  } else if (packToken) {
    tokenHash = createHash('sha256').update(packToken).digest('hex')
  ...
  if (maxPack && !/^[A-Za-z0-9_-]{8,120}$/.test(maxReadNonce)) {
    return res.status(400).json({ error: 'invalid_max_read_nonce' })
  }
  ...
  if (tokenResult.cached && tokenResult.result_json) {
    ...
    const delivery = await deliverCompletedTimeline({
      ...
      deliveryEmailHash,
      result: tokenResult.result_json,
      pack: isPack ? { packToken: maxPack ? null : packToken, creditsRemaining: credits.creditsRemaining, product: maxPack ? 'max3' : 'pack3' } : null,
    })
