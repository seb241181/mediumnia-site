...
export async function handleGiftCards(req, res, action) {
  ...
  if (action === 'create') {
    ...
    if (!people) {
      console.warn('[gift-cards] validation failed:', field)
    ...
    if (!offer || !practitionerId) {
      console.warn('[gift-cards] invalid offer:', { kind: req.body?.kind, amountCents: req.body?.amountCents, serviceId: req.body?.serviceId, practitioner: Boolean(practitionerId) })
