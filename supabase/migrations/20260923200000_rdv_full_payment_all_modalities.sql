-- MediumIA RDV — le paiement intégral en ligne s'ouvre aux rendez-vous en présentiel.
--
-- Migration préparée, à appliquer manuellement par le propriétaire (SQL Editor Supabase).
--
-- Jusqu'ici create_rdv_full_payment_hold refusait tout ce qui n'était pas en visio : un
-- client en présentiel ne pouvait payer en ligne que les arrhes (20 €), montant trop bas
-- pour le paiement PayPal en 4X (à partir de 30 €). La fonction accepte désormais la
-- modalité choisie ('video' ou 'in-person') dès qu'elle fait partie des modalités du
-- service. Toutes les autres protections sont inchangées (service instantané avec arrhes,
-- prix > 0, EUR, verrou praticien, idempotence du checkout, passage par
-- create_rdv_deposit_hold pour la disponibilité).
--
-- Tant que cette migration n'est pas appliquée, le site propose l'option mais la base
-- répond « invalide » pour le présentiel : le client voit un message et garde les arrhes.
...
BEGIN
  ...
  IF FOUND THEN
    ...
    IF v_existing_hold.id IS NULL
      ...
      OR v_existing_hold.selected_modality <> p_selected_modality
