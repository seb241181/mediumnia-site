...
export default function ConferenceLivePage() {
  ...
  return (
    <div className="min-h-screen bg-cream text-deep">
      <header className="border-b border-gold/20 bg-deep text-cream"><div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-5 py-5"><div><p className="font-georgia text-[10px] uppercase tracking-[.24em] text-gold">MEDIUMIA · LIVE</p><p className="mt-1 font-georgia text-sm text-cream/65">23 octobre · 19 h → 20 h</p></div><img src="/images/brand/MEDIUMIA_symbol_header.png" alt="MediumIA" className="h-10 w-auto" /></div></header>

      <main className="mx-auto max-w-6xl px-5 py-9 md:py-12">
        <section className="rounded-3xl bg-deep p-7 text-cream shadow-xl md:p-10"><p className="font-georgia text-xs uppercase tracking-[.22em] text-gold">Bonsoir {data?.participant?.firstName}</p><h1 className="mt-4 max-w-3xl font-georgia text-3xl font-medium leading-tight md:text-5xl">{data?.event?.title}</h1><div className="mt-6 flex flex-wrap gap-3"><span className={`rounded-full px-4 py-2 font-georgia text-xs ${data?.liveOpen ? 'bg-gold text-deep' : 'border border-gold/35 text-gold'}`}>{data?.liveOpen ? '● LIVE ouvert' : 'LIVE le 23 octobre à 19 h'}</span>{data?.event?.zoomJoinUrl && <a href={data.event.zoomJoinUrl} target="_blank" rel="noreferrer" className="rounded-full border border-cream/20 px-4 py-2 font-georgia text-xs text-cream">Ouvrir Zoom ↗</a>}</div></section>
