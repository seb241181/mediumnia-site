...
async function deliverCompletedTimeline({ supabase, drawTable, drawId, deliveryEmail, deliveryEmailHash, result }) {
...
export async function handleOracleTimeline(req, res) {
  ...
  const credits = isPack ? { creditsRemaining: tokenResult.credits_remaining, creditsTotal: tokenResult.credits_total } : {}
  if (tokenResult.cached && tokenResult.result_json) {
    ...
    return res.status(200).json({ ...tokenResult.result_json, ...credits, delivery })
  }
  ...
  try {
    ...
    return res.status(200).json({ ...responsePayload, ...credits, delivery })
  } catch (error) {
    ...
    if (drawClaimedProcessing) {
      try {
        const resetResult = isPack
          ? await supabase.rpc('release_chronosphere_pack_credit', { p_draw_id: drawId, p_failure_code: 'timeline_engine_failed' })
