...
const PREVIEW_TEST_EMAIL = 'sebastien.medium.59+conference-e2e@gmail.com'
const PREVIEW_TEST_SLUG = 'premiere-conference-mediumia'
const PREVIEW_TEST_OFFER_CENTS = 39900
const PREVIEW_TEST_SUPABASE_REF = 'wnbwhnqiulsdjcvkuwos'
...
function isPreviewTestEnvironment() {
  return process.env.VERCEL_ENV === 'preview'
    && String(process.env.SUPABASE_URL || '').includes(PREVIEW_TEST_SUPABASE_REF)
}

async function issuePreviewTestPass(req, res) {
  if (!isPreviewTestEnvironment()) return res.status(404).json({ error: 'not_found' })
  ...
  const { error: offerError } = await supabase
    .from('conference_events')
    .update({
      pass_offer_enabled: true,
      pass_offer_amount_cents: PREVIEW_TEST_OFFER_CENTS,
      pass_offer_currency: 'EUR',
      pass_offer_label: 'Offre conférence TEST',
      pass_duration_hours: 720,
      updated_at: new Date().toISOString(),
    })
    .eq('id', event.id)

  if (offerError) throw new Error('test_offer_prepare_failed')

  const { data: registration, error: registrationError } = await supabase
    .from('conference_registrations')
    .select('id,status')
    .eq('event_id', event.id)
    .eq('email_normalized', PREVIEW_TEST_EMAIL)
    .maybeSingle()

  if (registrationError || !registration || registration.status === 'cancelled') {
    throw new Error('test_registration_missing')
  }
  ...
  const { data: existing, error: existingError } = await supabase
    .from('conference_passes')
    .select('id,status,paypal_order_id,paypal_capture_id,redeemed_at')
    .eq('event_id', event.id)
    .eq('registration_id', registration.id)
    .maybeSingle()

  if (existingError) throw new Error('test_pass_lookup_failed')
  if (existing?.paypal_order_id || existing?.paypal_capture_id || existing?.redeemed_at) {
    return res.status(409).json({ error: 'test_pass_already_in_checkout' })
  }
  ...
  if (existing) {
    const { data, error } = await supabase
      ...
      .select('id')
      .single()
    if (error) throw new Error('test_pass_prepare_failed')
    pass = data
  } else {
    const { data, error } = await supabase
      .from('conference_passes')
  ...
  await supabase.from('conference_pass_events').insert({
    event_id: event.id,
    pass_id: pass.id,
    registration_id: registration.id,
    event_name: 'pass_issued',
    metadata: { mode: 'preview_test' },
  })

  return res.status(200).json({
    ok: true,
    passToken: rawToken,
    eventSlug: PREVIEW_TEST_SLUG,
    expiresAt,
  })
}

async function testPreviewIdempotenceReplay(req, res) {
  if (!isPreviewTestEnvironment()) return res.status(404).json({ error: 'not_found' })
  ...
  const { data: event } = await supabase
    ...
    .eq('slug', PREVIEW_TEST_SLUG)
    .maybeSingle()
  if (!event) throw new Error('event_lookup_failed')
  ...
  const { data: registration } = await supabase
    ...
    .eq('email_normalized', PREVIEW_TEST_EMAIL)
    .maybeSingle()
  if (!registration) throw new Error('test_registration_missing')

  const { data: beforePass } = await supabase
    .from('conference_passes')
    .select('id,status,paypal_order_id,paypal_capture_id,redeemed_at,entitlement_id,user_id,expires_at')
    .eq('event_id', event.id)
    .eq('registration_id', registration.id)
    .maybeSingle()

  if (!beforePass?.paypal_order_id || !beforePass?.paypal_capture_id || !beforePass?.redeemed_at) {
    return res.status(409).json({ error: 'test_pass_not_redeemed' })
  }

  const [{ count: beforePaymentEvents }, { count: beforeRedeemEvents }, { data: beforeEntitlement }] = await Promise.all([
    supabase.from('conference_pass_events').select('id', { count: 'exact', head: true }).eq('pass_id', beforePass.id).eq('event_name', 'payment_completed'),
    supabase.from('conference_pass_events').select('id', { count: 'exact', head: true }).eq('pass_id', beforePass.id).eq('event_name', 'pass_redeemed'),
    supabase.from('mediumia_entitlements').select('id,origin_ref,access_level,max_module,access_expires_at').eq('id', beforePass.entitlement_id).maybeSingle(),
  ])
  ...
  await captureCheckout(req, replayRes, beforePass.paypal_order_id)

  const { data: afterPass } = await supabase
    .from('conference_passes')
    .select('id,status,paypal_order_id,paypal_capture_id,redeemed_at,entitlement_id,user_id,expires_at')
    .eq('id', beforePass.id)
    .maybeSingle()

  const [{ count: afterPaymentEvents }, { count: afterRedeemEvents }, { data: afterEntitlement }, { count: originEntitlementCount }] = await Promise.all([
    supabase.from('conference_pass_events').select('id', { count: 'exact', head: true }).eq('pass_id', beforePass.id).eq('event_name', 'payment_completed'),
    supabase.from('conference_pass_events').select('id', { count: 'exact', head: true }).eq('pass_id', beforePass.id).eq('event_name', 'pass_redeemed'),
    supabase.from('mediumia_entitlements').select('id,origin_ref,access_level,max_module,access_expires_at').eq('id', beforePass.entitlement_id).maybeSingle(),
    supabase.from('mediumia_entitlements').select('id', { count: 'exact', head: true }).eq('origin_ref', beforeEntitlement?.origin_ref || '__missing__'),
  ])

  const unchanged = {
    sameCapture: afterPass?.paypal_capture_id === beforePass.paypal_capture_id,
    sameOrder: afterPass?.paypal_order_id === beforePass.paypal_order_id,
    sameRedeemedAt: afterPass?.redeemed_at === beforePass.redeemed_at,
    sameEntitlement: afterPass?.entitlement_id === beforePass.entitlement_id,
    sameAccessExpiry: afterEntitlement?.access_expires_at === beforeEntitlement?.access_expires_at,
    paymentEventCount: beforePaymentEvents === afterPaymentEvents,
    redeemedEventCount: beforeRedeemEvents === afterRedeemEvents,
    singleOriginEntitlement: originEntitlementCount === 1,
  }

  return res.status(200).json({
    ok: Object.values(unchanged).every(Boolean),
    replayStatus,
    alreadyProvisioned: replayPayload?.alreadyProvisioned === true,
    accessProvisioned: replayPayload?.access?.status === 'provisioned',
    unchanged,
    counts: {
      paymentCompleted: afterPaymentEvents || 0,
      passRedeemed: afterRedeemEvents || 0,
      originEntitlements: originEntitlementCount || 0,
    },
  })
}
...
export async function handleConferencePassPayPal(req, res, action) {
  ...
  try {
    if (action === 'test-issue') {
      return await issuePreviewTestPass(req, res)
    }

    if (action === 'test-idempotence') {
      return await testPreviewIdempotenceReplay(req, res)
    }
