...
function validateOrderAgainstIntent(cfg, data, intent, { requireCaptured = false } = {}) {
  ...
  if (unit.amount?.currency_code !== intent.currency || unit.amount?.value !== expectedAmount) {
  ...
  if (requireCaptured && data.status !== 'COMPLETED') throw new Error('paypal_capture_incomplete')
  return unit
}
