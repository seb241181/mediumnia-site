...
async function createCheckout(req, res, cfg, tokenHash) {
  ...
  if (!reserved.data?.ok) throw new Error(reserved.data?.reason || 'pass_reserve_failed')

  return res.status(201).json({ id: data.id, env: cfg.env })
}
  ...
  try {
    if (action === 'config' || action === 'validate' || action === 'create') {
      ...
      if (action === 'validate' || action === 'config') {
        ...
        return configResponse(req, res, pass)
      }

      if (req.method !== 'POST') return res.status(405).json({ error: 'method_not_allowed' })
      if (!pass.offerEnabled || !pass.offerAmountCents) return res.status(404).json({ error: 'offer_disabled' })
      const cfg = runtimeConfig(pass.offerAmountCents)
      return createCheckout(req, res, cfg, tokenHash)
    }

    if (action === 'capture') {
      if (req.method !== 'POST') return res.status(405).json({ error: 'method_not_allowed' })
      return captureCheckout(req, res, normalizeOrderId(req.body?.orderId))
    }
