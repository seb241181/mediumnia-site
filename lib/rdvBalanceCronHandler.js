...
import { sendReviewRequests } from './rdvReviewRequests.js'
import { sendDueGiftCards } from './giftCards.js'
import { sendDefiReminders } from './defiIntuitionServer.js'
import { syncLiveSubscriptions } from './formationPath.js'
import {
  balancePaymentUrl,
...
export async function handleRdvBalanceDailyCron(req, res) {
  ...
  try {
    ...
    const result = await runDailySweep(supabase)
    console.info(`[rdv-balance-cron] candidates=${result.candidates} counts=${JSON.stringify(result.counts)}`)
    // Review requests run after the balance sweep and never make it fail.
    let reviews
    try {
      reviews = await sendReviewRequests(supabase)
      console.info(`[rdv-review-requests] ${JSON.stringify(reviews)}`)
    } catch (error) {
      reviews = { error: error?.message || 'unknown_error' }
      console.error(`[rdv-review-requests] failed: ${reviews.error}`)
    }
    // Gift cards whose chosen delivery date has come.
    let giftCards
    try {
      giftCards = await sendDueGiftCards(supabase)
      console.info(`[gift-cards-due] ${JSON.stringify(giftCards)}`)
    } catch (error) {
      giftCards = { error: error?.message || 'unknown_error' }
      console.error(`[gift-cards-due] failed: ${giftCards.error}`)
    }
    // Défi Intuition: daily reminder for those who asked for it.
    let defi
    try {
      defi = await sendDefiReminders(supabase)
      console.info(`[defi-reminders] ${JSON.stringify(defi)}`)
    } catch (error) {
      defi = { error: error?.message || 'unknown_error' }
      console.error(`[defi-reminders] failed: ${defi.error}`)
    }
    // Parcours au mois: catch any PayPal payment whose notification was missed.
    let formationPath
    try {
      formationPath = await syncLiveSubscriptions(supabase)
      console.info(`[formation-path-sync] ${JSON.stringify(formationPath)}`)
    } catch (error) {
      formationPath = { error: error?.message || 'unknown_error' }
      console.error(`[formation-path-sync] failed: ${formationPath.error}`)
    }
    return res.status(200).json({ ok: true, candidates: result.candidates, counts: result.counts, reviews, giftCards, defi, formationPath })
  } catch (error) {
